...
export function PolicyAnalysis({ policyFile, policyAnalysisComplete, navigate }: { policyFile: PolicyFile | null; policyAnalysisComplete: boolean; navigate: (screen: Screen) => void }) {
  ...
  if (!policyFile || !fixture || !policyAnalysisComplete) return <div className="page-stack policy-analysis-page"><PageHeader eyebrow="Policy analysis" title="No verified analysis available" description="Analyse a prepared Northstar recruitment policy before reviewing its Policy IR and cited controls." /><Panel><div className="locked-empty"><FileText size={27} /><h2>Analysis is locked</h2><p>Unknown documents never receive prepared findings in this offline demonstration.</p><Button onClick={() => navigate('policies')} icon={<ArrowLeft size={16} />}>Open policy intake</Button></div></Panel></div>

  return (
    <div className="page-stack policy-details-page policy-analysis-page">
      <PageHeader eyebrow={`Policy IR · ${fixture.documentId} · v${fixture.version}`} title="Cited policy analysis" description="Trace each source clause through normalization, Agent Manifest mapping, and the compiled runtime decision." actions={<><Badge tone={fixture.status === 'Active' ? 'success' : 'violet'} dot>{fixture.status}</Badge><Badge tone="neutral">Prepared content-verified analysis</Badge></>} />
