  ...
  return (
    <div className="page-stack policy-patch-page">
      ...
      {workflowStage < 3 ? (
        ...
        <>
          <div className="policy-patch-version"><span>Policy version</span><strong>v1.4</strong><ArrowRight size={18} /><strong>v1.5</strong></div>
      ...
      {workflowStage >= 4 && <Panel title="Policy retest results" description="The incident scenario is now included in representative coverage testing." action={<Badge tone="success">100% coverage</Badge>}><div className="policy-patch-retest-results"><section><header><span>Before</span><strong>Policy v1.4</strong></header>{incident.retest.before.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</section><ArrowRight size={21} /><section className="after"><header><span>After</span><strong>Policy v1.5</strong></header>{incident.retest.after.map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</section></div></Panel>}

      {workflowStage >= 5 && <div className="policy-patch-approved"><div className="policy-patch-approved-icon"><Sparkles size={25} /></div><div><Badge tone="success" dot>Policy v1.5 approved</Badge><h2>Policy v1.5 approved</h2><p>Simulated prototype remediation · Representative policy patch</p></div><dl><div><dt>Policy version</dt><dd>{incident.approval.policyVersion}</dd></div><div><dt>Patch</dt><dd className="mono">{incident.approval.patch}</dd></div><div><dt>Approved by</dt><dd>{incident.approval.approvedBy}</dd></div><div><dt>Status</dt><dd>{incident.approval.status}</dd></div></dl></div>}
